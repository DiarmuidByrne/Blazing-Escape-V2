...
public class AzureUILeaderboard : MonoBehaviour {
	...
	public string AzureEndPoint = "https://blazing-escape.azure-mobile.net/";
	public string ApplicationKey = "AwnWTrrLcEACaIintQpyouKwtonMQI49";
	public string FacebookAccessToken = "CAAXY9hlYRyQBACvUuN1hNsETZAPuGEgn0JLElZCigzmYtsbaL3Y4bfotfoc6cFN3ZA4wDzXtUn98jCoygfxDXh0gG7Ies4RVE7544vHJnTRGZANew9veAQe3xaMVaQYipVxwlZBM3ZBPK3BivFbtaEIRJGYBTsZAzSzZBdCzZBhn9WwIaEyV0wXrCRzm4gmMqdU8ZD";
	...
	void Start() {
		...
		// Creates new Highscore variable in PlayerPrefs
		if (!PlayerPrefs.HasKey("PlayerHighScore")) {
			PlayerPrefs.SetInt("PlayerHighScore", 0);
		}
		score = PlayerPrefs.GetInt("PlayerHighScore");

		// If no name exists, create new name
	...
	public void GetAllItems() {
		_rankingList.Clear();
		azure.Where<Ranking>(p => p.PlayerName != null, ReadHandler);
	}

	public void ReadHandler(AzureResponse<List<Ranking>> response) {

		var list = response.ResponseData;

		Debug.Log("Items ==================");
		foreach (var item in list) {
			Debug.Log((item.PlayerHighScore.ToString()) + "," + item.PlayerName + "," + item.Id);
	...
	public void OnGUI() {
		...
		foreach (var item in _rankingList) {
			...
			GUILayout.Label((item.PlayerHighScore.ToString()));
